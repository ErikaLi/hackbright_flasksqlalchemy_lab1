...
def make_new_student(first_name, last_name, github):
    ...
    """
    QUERY_DICT = {'first_name': first_name,
                'last_name': last_name,
                'github': github}

    db.session.execute(QUERY, QUERY_DICT)
    db.session.commit()
...
def get_project_by_title(title):
    ...
    cursor = db.session.execute(QUERY, QUERY_DICT)
    row = cursor.fetchone()

    print "Project info: {description}" .format(description=row[0])
